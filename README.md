...
It implements Leslie Lamport's logical clock protocol to establish partial ordering of events in distributed systems. The implementation includes both the core `clock::lamport_clock::LamportClock` data structure and a multi-threaded simulation framework that demonstrates the clock's behavior in a network of processes communicating via message passing. On top of that, it also implements vector clocks in the `clock::vector_clock::VectorClock` data structure, which overcomes some limitations of Lamport clocks in capturing causality.
